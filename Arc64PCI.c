...
|   08-Apr-2020 sds  3.6    Initial                                            							|
|                                                                              							|
|   Development notes:                                                         							|
+-------------------------------------------------------------------------------------------------------+
|   This driver has been tested on CentOS 8 x64                               							|
|                                                                              							|
+-------------------------------------------------------------------------------------------------------+
*/
...
static ulong			g_ulNextStartAddr	= 0;
...
static int Arc64_open( struct inode* inode, struct file* filp )
{
	...
	//	//
	//	// IMPORTANT NOTE: This prevents a user reported error (Matt Nelson) where the PCI master bit
	//	//                 is not properly set for the bus on some systems. This results in successful
	//	//                 commanding ( I think ) but no image transfer.
	//  //
        ...
        switch ( dCtrlCode )
		{
        	...
        	*/
			case ASTROPCI_COMMAND:
			{
				...
				if ( copy_to_user( ( uint32_t * ) arg, u32CmdData, sizeof( uint32_t ) ) )
				{
					dResult = -EFAULT;
				}
			}
...
static struct cluster* add_alloc( struct cluster_set* set, struct page* new_chapter )
{
	...
	{
		struct cluster *new_cluster = kmalloc( sizeof( *new_cluster ), GFP_KERNEL );
